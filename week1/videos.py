...
def video_with_bbox(path, netw_name, gtInfo, predictionsInfo):
    ...
    while cap.isOpened():
        ...
        print('Read a new frame ' + str(count) + '): ', ret)
        if not ret:
            print("Can't receive frame (stream end?). Exiting ...")
            break
        aux1 = gtInfo[count].get('bbox').astype(int)
        for bb in aux1:
            frame = cv.rectangle(frame, (bb[0], bb[1]), (bb[2], bb[3]), (0, 255, 0), 1) # GT - green
