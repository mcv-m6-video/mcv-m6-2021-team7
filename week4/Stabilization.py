...
from mesh_src.Optimization import real_time_optimize_path
from mesh_src.MeshFlow import motion_propagate, mesh_warp_frame, generate_vertex_profiles
...
def read_video(cap):
    ...
    # path parameters
    shape = (int(old_frame.shape[0] / PIXELS), int(old_frame.shape[1] / PIXELS), 1)
    x_paths = np.zeros(shape)
    y_paths = np.zeros(shape)
...
def plot_vertex_profiles(x_paths, sx_paths, output_pah):
    """
...
def generate_stabilized_video(cap, x_motion_meshes, y_motion_meshes, new_x_motion_meshes, new_y_motion_meshes):
    ...
    out = cv2.VideoWriter('mesh_stable.avi', fourcc, frame_rate, (2 * frame_width, frame_height))
            ...
            for i in range(x_motion_mesh.shape[0]):
                for j in range(x_motion_mesh.shape[1]):
                    ...
                    cv2.line(frame, (j * PIXELS, i * PIXELS),
                             (int(j * PIXELS + r * np.cos(theta)), int(i * PIXELS + r * np.sin(theta))), 1)
...
def mesh_flow_main(cap, out_path, video_percentage):
    start_time = time.time()

    # propagate motion vectors and generate vertex profiles
    x_motion_meshes, y_motion_meshes, x_paths, y_paths, HORIZONTAL_BORDER, VERTICAL_BORDER = read_video(cap,
                                                                                                        video_percentage)
    ...
    # plot_vertex_profiles(x_paths, sx_paths, out_path)

    # get updated mesh warps
    x_motion_meshes, y_motion_meshes, new_x_motion_meshes, new_y_motion_meshes = get_frame_warp(x_motion_meshes,
                                                                                                y_motion_meshes,
                                                                                                x_paths, y_paths,
                                                                                                sx_paths, sy_paths)

    # apply updated mesh warps & save the result
    generate_stabilized_video(cap, out_path, video_percentage,
                              x_motion_meshes, y_motion_meshes, new_x_motion_meshes, new_y_motion_meshes,
